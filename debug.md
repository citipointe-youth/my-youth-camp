...
| `RENDER.pushActivity` (Notification delivery, 2026-09-22) + the All/Not-logged-in / All/Not-receiving filter on both screens | grep `/* ===== NOTIFICATION DELIVERY ===== */`. Empty/403 → `GET /push/devices` (`push.controller devices`, `admin:manage`). "Every row says not yet delivered" → **expected until the first urgent push/test** (`lastSuccessAt` is only set by a real send). Phone type "Unknown device" → a pre-`0027` subscription whose phone hasn't reopened the app yet (`_pushLabelSync` → `POST /push/label`). **Subscribe suddenly failing with "Validation failed"** → `_deviceLabel` produced a value outside `PUSH_DEVICE_LABELS` (`node scripts/push-activity-harness.js`). Every push write failing after deploy → migration `0027` not applied. Filter stuck → `_laOnlyMissing`/`_paOnlyMissing`. Phone shows no initials → that login never sets them (non-church) or the phone hasn't reopened since `0028`; initials stale after a device changes hands → `_saveLeaderInitials`'s `_pushLabelSync(true)`. A LEADERSHIP phone reads `Liam · iPhone` from the account's `firstName` (`devLine`'s `fallbackName`, `leader` rows only — never church, whose holder name is the church); two phones on one leadership account legitimately read alike. **A phone's history dropdown missing/short** → `delivery_history` is written ONLY by `recordSuccess` (atomic SQL), capped at `MAX_DELIVERY_HISTORY`, and is CLEARED when the phone is re-subscribed under a different account; an ordinary `save()` deliberately leaves it alone, so a lost entry means `recordSuccess` was bypassed, not a race. |
| The Notices alerts card says **"Alerts on this phone are from `<other account>`"** (2026-09-22) | Correct, not a bug: a subscription belongs to whichever login last turned alerts on, and `logout()` never unsubscribes. `_pushSwitch` moves it (ordinary `/push/subscribe` upsert, clears that phone's history). The owner name is remembered on the PHONE (`ycp_push_owner`) — set by `_pushFinish`/`_pushSwitch`, cleared by `_pushOff`, learned once from `POST /push/label`'s `owner`. Card shows the OLD wording when the owner is unknown (never guesses). Wrong name after an account swap → the phone hasn't re-synced; stale "alerts are on" while the server row was pruned → known pre-existing gap (the card only reads the phone's local subscription). |
...
| Accommodation | `src/services/accommodation.service.ts` + `accommodation-allocation.ts` (pure: groups/validation/tents) | classroom rooms CRUD, allocation map, 75% eligibility, single-gender/capacity validation, lock, church-rooms. **No blocks/reservations** (removed). **(2026-09-24)** `isEligible(c, opts)` takes optional `EligibilityOptions{perRegistration}`; `eligibilityOptions()` builds the set from `churches.accommodation_per_registration`; `listGroups`/`setAllocations` both pass it through. `setPerRegistration` (`PATCH /accommodation/per-registration/:churchId`, director/admin, lock-gated) toggles the flag and, only when switching OFF, prunes that church's now-orphaned `classroom_allocations` rows via a fresh `computeGroups`. SPA mirror `_accomEligible(c)`/`ACCOM_ELIGIBLE_RATIO`. |
...
| Symptom | Go to |
|---|---|
| **A person who bought two tickets reads short by the second invoice's amount** | `invoiceNumbers` (migration `0023`), written by `ticket-import.service.ts` and indexed by tier 1 in `invoice-import.service.ts`. `Person.invoiceNumber` is a SCALAR and the second ticket overwrites it, so before 2026-09-09 the other invoice fell to the billing-contact-name tier and was lost whenever a PARENT paid. Check the person actually has both numbers in `invoice_numbers`; if the array is null they have not been through a Ticket List import since the migration. |
| **An invoice is unmatched and the billing contact is obviously a parent** | Same fix. Tier 2 (billing name) only ever worked when the payer WAS the registrant — that is why Froneman/Nel/Gao were fine and Dylan Foo was not. On the 2026-09-08 export the fixed pipeline matches all 614 invoices on tier 1 and uses tier 2 **zero** times. If tier 2 is firing again, the ticket→invoice link is not being stored. |
| **A person on a family invoice lost the money from their own separate invoice** | The shared-invoice second pass used to ASSIGN `incoming.amountPaid`; it now goes through the shared `accumulate()` like the singles pass. ⚠️ Fingerprint: a `discountCode` present with `discount_amount 0` — the code survives the singles pass while its value gets overwritten by the shared split. Do NOT reintroduce a direct assignment in that loop, and do not fold the two passes into one (they are separate so prices are learned before family invoices are split). |
| **`multiple-invoices-summed` did not fire for someone who clearly has two invoices** | Before 2026-09-09 the two passes counted in different structures, so a shared + single combination was invisible to the warning. Both now share `moneyByPerson`. |
| **A discount code shows a large "discount given" but no revenue was actually lost** | It is an upgrade code — the discount offsets a SECOND ticket bought to replace a first that was already paid in full. Tag it `upgrade` on the Budget screen. Measured 2026-09-08: `YC26CLASS` + `YC26CLASSFULL` reported $770 against $0 genuinely foregone. |
| **An `upgrade`-tagged code's money changed** | It should not have. The tag changes REPORTING only — `receivedBeforeRefund`'s cascade was always correct for these people. Only `sponsor` has a forcing rule; if a second one appeared, that is the 2026-08-05 bug class re-opening. |
| **Every discount-code tag reverted to plain / a whole tag map emptied itself** | `updateDiscountCodeTags` (`settings.service.ts`) whitelists tag values and saves `clean` as the ENTIRE map, so any value missing from that whitelist is wiped from every code on the next unrelated tag edit. Check the whitelist lists all four `DiscountTag` values. This bit `upgrade` on 2026-09-09. |
| **An `upgrade` code turned up under "⚠ Not classified"** | `_discountTagFor` / `discountTagFor` stopped recognising `'upgrade'`. `isUnclassifiedDiscount` returns false only while the tag is a recognised value, so dropping it from that guard makes the code read as unknown money. Both copies (`budget.ts` and the SPA mirror) must list all four tags. |
| **"Upgrade paid" is empty while people are listed as owing** | `_budUpgrades` excludes any class that is not `classroom` or `classroom-upgrade`. Before 2026-09-09 it excluded every tagged class, so the two people who verifiably HAD paid were filtered out and three who paid invisibly sat under "outstanding". |
| **The Carindale cohort still shows as "Upgrade outstanding"** | Known and deliberate as of 2026-09-09 (owner's call: *"may or may not pay an upgrade depending on their situation"*). The undetermined list lives in the import warnings as the LAST group (`accommodation-church-override`, severity `note`), not as a Budget bucket. The card is unchanged and still asserts $320 across 10 people. |
| **A warning group sorts in the wrong place / a new severity is ignored** | `_WARN_SEV_ORDER` in `public/index.html` mirrors the severity union in `src/core/types/import-warning.ts`. `note` is rank 3, BELOW `info`, so it always sorts last. A severity present in one copy and not the other falls back to rank 1 and sorts as if it were `review`. |
| **`accommodation-church-override` reads as an ordinary info line again** | Its CODE STRING is deliberately unchanged (out-of-repo contract with the upload machine) — only label/severity/message moved to `note` + "Upgrade status undetermined". Do not "fix" it by adding a parallel code; that double-reports the same rows. |
| **A test breaks on the wording of an import warning** | It is asserting the prose. `message` is explicitly free-text; assert `w.code` instead. This is what happened to `ticket-import.service.test.ts` on the 2026-09-09 reword. |
| **`refund-likely` fired for someone who simply has not paid** | The rule requires the discount to cover the WHOLE ticket (`discountAmount >= ticketTotal`), not merely `amountPaid === 0`. An unpaid invoice is money still owed, not a comp. If unpaid invoices are being flagged, that test has been weakened. |
| **`refund-likely` did not fire for someone comped after paying on a family invoice** | The ledger must be recorded in BOTH passes. Ava Riverstone's paid place is a share of a shared invoice, so a singles-only ledger misses her entirely. |
| **A `refund-likely` person got flagged `needsReview`, or their money moved** | Neither is intended. The warning REPORTS; the payment genuinely arrived and keeps counting until a real refund is recorded (`refundAmount`, migration `0022`). `needsReview` means "the import is unsure", and this import is certain. |
...
| **An "Add dual-gender login" account (`all-<slug>`) never comes back after new-year rollover, even though it existed when Save Defaults was last run** | Working as designed (2026-09-14, migration `0024`) — this is the ONE exception to the row above. `admin.service.ts` `saveDefaults()` explicitly filters out every `is_dual_gender_login` user before building the snapshot, regardless of timing, so it can never survive `newYear()`. That is the whole point of the feature: it reverts to boy/girl-only accounts every rollover. If it should be permanent scaffold instead, that filter is the one line to remove — but that is a real feature reversal, not a bug fix. |
...
| **A church login sees a parent number in cleartext** (or can't reveal one) | `PARENT_PHONE_MASKED_ROLES` in `camper.controller.ts` = `{firstAid, church}` — **`/campers` only.** ⚠️ `registrant.controller.ts` (`/registrants`) applies NO masking at all, on purpose (owner declined 2026-09-21, see CLAUDE.md) — a cleartext parent number there is not a bug. The mask is server-side by design — client-side hiding writes no audit row. SPA side: `_parentPhoneCell` decides by looking for `*` in the value, NOT by role, so it already renders correctly either way. |
...
| **Saving room placements fails with `Unknown group` / `Allocated more than available`** | **Obsolete since 2026-09-24 (2nd) — stored staleness self-heals.** `applyAllocationRequest` keeps untouched groups' healed stored placements whatever the request carries, so a re-split, un-split, cancellation or under-75% drop can no longer fail a save. If one of these errors DOES reach the user now (as a 400 toast), it is about a group the request actually CHANGED: `Unknown group … Refresh and try again` = the page was loaded before that group's key changed (refresh); `more than available` = the page thinks the group is bigger than it is. Healed entries are listed in the warnbox on the allocations screen and in the `healed` array of `GET /accommodation/state`. |
| **Allocations screen: "Some placements were cleared — re-place them"** | Working as designed (2026-09-24 auto-heal). A stored placement no longer matches a live group — reason is in the notice (re-split at 50 / group shrank / ministry no longer eligible / room removed). Reads don't clear it; the next add/remove persists the clean-up. Dismiss is per device (`localStorage.ycp_accom_heal_dismissed`, keyed by the notice's content). |
| **A room shows 29/25 in red, or a church sees more in a room than was placed** | Soft freeze (2026-09-24, `classroom_freeze`): late registrations are absorbed into their group's rooms past capacity, by design. Effective counts are computed at read time (`effectiveAllocations` / SPA `_accomEffective`) — the stored `classroom_allocations.n` stays at the pre-freeze value until Unfreeze writes the effective counts back. Church view caches 30s per instance. |
| **After Unfreeze, a group's placements vanished** | The pool grew past 50 while frozen, so on unfreeze it re-split under the normal rules and the old whole-pool placement was healed away (reported in the notice). Re-place the new year-bracket groups. |
| **"Classrooms are soft-frozen — a ministry cannot gain or lose classroom eligibility"** | `setPerRegistration` refused while `classroom_freeze` has a row. Unfreeze first. |
...
| **An "Add dual-gender login" account (`all-<slug>`) got deleted by a password rotation / church split, or a church rename stripped its `all-` prefix** | `is_dual_gender_login` (`users` table, migration `0024`, 2026-09-14). It is `role:'church'` + `genderScope: null` — the EXACT shape `retireLegacyChurchLogins` (`account.service.ts`) treats as a legacy pre-split login to delete, so it MUST be excluded there (it is). It is also deliberately excluded from `_churchAccts()`/`editChurchName`/`bulkChurchUpdate` in `public/index.html` — those re-apply a `b-`/`g-` prefix to every account they find (`_churchPrefix` returns `''` for a non-gender-scoped account), so including it there would rewrite `all-<slug>` down to bare `<slug>` on the next church rename. If either exclusion is ever "cleaned up," this is exactly what breaks. |
