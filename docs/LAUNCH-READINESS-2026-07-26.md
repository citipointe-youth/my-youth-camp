...
| B8 | ~~Decide the church password keyspace~~ — **RESOLVED 2026-07-31: widened `memorablePassword` to `Word.###`** (~117k, was ~11.7k) | The owner chose the three-digit form. Existing hashed passwords stay valid; only newly generated ones use it, so re-run "Randomise & export church passwords" to move the churches onto the wider keyspace | Done |
