...
| **R9** | **Observability review.** Ship error tracking (F5); add a DB-touching readiness route and point an external monitor with SMS alerting at **that**, not `/health` (F4). | With privacy and device UX already confirmed, this is now **the top code item**. It converts "three churches quietly gave up on Saturday" into a signal you actually receive — today the only signal is someone ringing you. | **Before Saturday** |
